...
export async function PATCH(req: Request, {params}:{params: { id : string}}){
    ...
    try {
        ...
        if(!project.teamMembers.includes(member._id)){
            return Response.json({ 
                success: false, 
                message: "Member not part of the project" 
            }, { status: 404 });
        }   

        project.teamMembers = project.teamMembers.filter((memberId: ObjectId) => memberId.toString() !== member._id.toString())
        await project.save()
        ...
        console.log("Error in removing member from projects", error)    
        return Response.json({
            success: false,
